...
void movement(Game *game)
{
    ...
    for (int i = 0; i < game->n; i++) {
        ...
        if (dist <= circle.radius) {
            ...
            p->s.center.x = (d2/dist) * circle.radius + circle.center.x + 1.0;
            p->s.center.y = (d1/dist) * circle.radius + circle.center.y + 1.0;

            if (p->velocity.x > 2.5 || p->s.center.x > circle.center.x) 
                p->velocity.x += 0.05;
            else 
                p->velocity.x -= 0.2;
            p->velocity.y -= 0.1;	
        ...
        if (p->s.center.y < 0.0 || p->s.center.y > WINDOW_HEIGHT ||
            p->s.center.x < 0.0 || p->s.center.x > WINDOW_WIDTH) {
